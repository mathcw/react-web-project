...
const add = (reload: () => void) => () => {
  ...
  const list = {
    pd_type: { text: "产品类型", required: true, type: "PdType" },
    name: { text: "标签名称", required: true }
  };
  const onSubmit = (data: any) => {
    submit("/business/ProductTheme/submit", data).then(r => {
...
const edit = (reload: () => void) => (ref: any) => {
  ...
  const onSubmit = (data: object | undefined) => {
    submit("/business/ProductTheme/submit", {id:ref.id,...data}).then(r => {
      message.success(r.message);
      modalRef.destroy();
      reload();
    });
