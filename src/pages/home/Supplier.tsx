...
const page:React.FC<IModPageProps> = ({ route }) => {
  ...
  const renderOrder=(order:any)=> {
    ...
    // if (order.type === '2') {
    //   return (
    //     <Traffic data={order} key={order.id} />
    //   )
    // }
    // if (order.type === '3') {
    //   return (
    //     <Booking data={order} key={order.id} />
    //   )
    // }
    // return (
    //   <Visa data={order} key={order.id} />
    // )
  }

  const handleClick =(index:any)=>{
    data.announceModal=true;
    data.announceData=data.announce[index];
    setData({ ...data });
  }

  const announceCancel= () => {
    data.announceModal=false;
    setData({ ...data });
  }
  ...
  return <React.Fragment>
              ...
              <Col className={styles.content}  style={{height:'auto'}}>
                {data.announce.map((item, index) => (
                  <Row key={index}>
                    ...
                    <Col span={18} className={[styles.itemcentertwo, 'text-overflow', styles.itemfocus].join(' ')} onClick={e => handleClick(index)}>
                      {item.title}
                    </Col>
                  </Row>
                ))}
        ...
        <Modal
          visible={data.announceModal}
          title="平台公告"
          onCancel={announceCancel}
          footer={[
            <Button key="back" onClick={announceCancel}>
              确定
            </Button>
          ]}
        >
          {data.announceModal && 
            ...
            <Col span={24} className={styles.title}>
              <span className={[styles.text1, 'text-overflow'].join(' ')}>{data.announceData.title}</span>
            </Col>
            <Col span={24} className={styles.title}>
              <span className={[styles.text, 'text-overflow'].join(' ')}>{data.announceData.company_name}</span>—
              <span className={[styles.text, 'text-overflow'].join(' ')}>{data.announceData.department_name}</span>—
              <span className={[styles.text, 'text-overflow'].join(' ')}>{data.announceData.employee_name}</span>
            </Col>
            <Col span={24} className="styles.title1" dangerouslySetInnerHTML={{__html: data.announceData.body_html}} />
          </Col>
          }
        </Modal>
    </React.Fragment>
